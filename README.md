...
把一份共享 skill 仓库同步到 Claude Code、OpenAI Codex 等 CLI agent 工具中，让 skill 只维护一份，多端自动可用。
...
- **一键同步：** 通过 Windows directory junction 同步到 Claude Code 和 OpenAI Codex。
...
1. 选择共享 skill 根目录，例如 `D:\AgentSkills`。
2. 点击 `Scan` 扫描包含 `SKILL.md` 的 skill 文件夹。
3. 选择 skill，并同步到 Claude Code 或 OpenAI Codex。
...
- 已支持 Claude Code 与 OpenAI Codex。
- Gemini CLI、OpenCode、Hermes 目前仅作为占位目标。
- 同步能力当前仅面向 Windows。
